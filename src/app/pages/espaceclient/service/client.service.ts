...
export class ClientService extends ClientInterface {
  ...
  client_exist(client: Client): Promise<any> {
    let promise = Promise((resolve, reject) => {
      this.http.get<any>(`${config.apiUrl}/bo/` + this.loginService.getJwtToken() + "/client_exist?s=" + client.nom_societe).toPromise()
        .then(data => {
          console.log("success");
          resolve(data);
        }, msg => {
  ...
  compte_exist(compte: Compte): Promise<any> {
    let promise = Promise((resolve, reject) => {
      this.http.get<any>(`${config.apiUrl}/bo/` + this.loginService.getJwtToken() + "/compte_exist?login=" + compte.login).toPromise()
        .then(data => {
          console.log("success");
          resolve(data);
        }, msg => {
...
update_client(client: Client): Promise<any> {
  console.log(client);
  let promise = Promise((resolve, reject) => {
    this.http.put<any>(`${config.apiUrl}/bo/` + this.loginService.getJwtToken() + "/update_client",client).toPromise()
      .then(data => {
        console.log("success");
        resolve(data);
...
desactiver(client: Client): Promise<any> {
  console.log(client);
  let promise = Promise((resolve, reject) => {
    this.http.put<any>(`${config.apiUrl}/bo/` + this.loginService.getJwtToken() + "/desactiver",client).toPromise()
      .then(data => {
        console.log("success");
        resolve(data);
