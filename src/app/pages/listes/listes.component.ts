...
import { Boitier, Brancher, Engine, Sim, Type_boitier, Type_engine, Type_operateur } from '../espaceboitier/service/Boitier';
...
editProfileForm: FormGroup;
  constructor(private modalService: NgbModal,private _formBuilder: FormBuilder,private clientService : ClientService,private boitierservice : BoitierService,private router:Router) { }
...
desactiver(client: Client){
  //localStorage.setItem("id",client.id.toString());
  this.clientService.desactiver(client).then(
    data=>{
      console.log(data); 
...
delete_client(id){
  let index=this.clients.findIndex(e=>e.id===id);
  if(index!== -1){
